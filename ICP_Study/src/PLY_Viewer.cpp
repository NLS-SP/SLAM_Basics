...
int main()
{
    ...
    pcl::io::loadPolygonFilePLY("/Users/robotics_qi/Data/mesh.ply", mesh);									//PCL利用VTK的IO接口，可以直接读取stl,ply,obj等格式的三维点云数据,传给PolygonMesh对象
    pcl::io::mesh2vtk(mesh, polydata);												//将PolygonMesh对象转化为vtkPolyData对象
    pcl::io::vtkPolyDataToPointCloud(polydata, *cloud);								//获取点云
    pcl::io::savePCDFileASCII("desktop.pcd", *cloud);									//存储为pcb文件



    char strfilepath[256] = "desktop.pcd";
