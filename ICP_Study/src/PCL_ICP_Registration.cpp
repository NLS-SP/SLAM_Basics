...
int main (int argc, char* argv[])
{
    ...
    time.tic ();
    if (pcl::io::loadPCDFile(source_cloud_path, *cloud_in) < 0){
        PCL_ERROR ("Error loading cloud %s.\n", argv[1]);
        return (-1);
    }
    std::cout << "\nLoaded file " << source_cloud_path << " (" << cloud_in->size () << " points) in " << time.toc () << " ms\n" << std::endl;

    if (pcl::io::loadPCDFile(target_cloud_path, *cloud_tr) < 0){
        PCL_ERROR ("Error loading cloud %s.\n", argv[1]);
        return (-1);
    }
    std::cout << "\nLoaded file " << source_cloud_path << " (" << cloud_in->size () << " points) in " << time.toc () << " ms\n" << std::endl;
