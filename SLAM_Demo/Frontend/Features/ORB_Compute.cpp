//
// Created by gatsby on 2019-02-09.
//
 ...
 *
 * Note: if a keypoint goes outside of the image boundary(8 pixels), descriptors will not
 *       be computed and will be left as empty.
 * */
...
int main()
{
    ...
    std::cout << "extract ORB cost= " << t_detectUsed.count() << " seconds. " << std::endl;
    ...
    std::cout << "Match ORB cost: " << t_matchUsed.count() << " seconds. " << std::endl;
...
void computeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors){
    ...
    for(auto &kp : keypoints) {
        ...
        // compute the angle: arcran(m01/m10);
    ...
    for(size_t i1= 0; i1 < desc1.size(); ++i1){
        ...
        for(size_t i2 = 0; i2 <desc2.size(); ++i2){
            ...
            for(int k = 0; k < 8; k++){
                ...
                // return the number of set bits in an integer(the nuymber of ones in the binary representation)
