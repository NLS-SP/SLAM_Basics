...
namespace LIV_OpenGL {
    ...
    template<>
    void GlCapabilities::initGlParameter<int>(GLenum name, uint32_t num_values) {
        int32_t v[4];
        glGetIntegerv(name, v);
        if (num_values == 1) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0])));
        if (num_values == 2) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0], v[1])));
        if (num_values == 3) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0], v[1], v[2])));
        if (num_values == 4) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0], v[1], v[2], v[3])));
    }

    template<>
    void GlCapabilities::initGlParameter<float>(GLenum name, uint32_t num_values) {
        float v[4];
        glGetFloatv(name, v);
        if (num_values == 1) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0])));
        if (num_values == 2) state_.insert(std::make_pair(name, GlState::GlStateVariable(v[0], v[1])));
    ...
    template<>
    void GlCapabilities::initGlParameter<bool>(GLenum name, uint32_t num_values) {
        GLboolean v[4];
        glGetBooleanv(name, v);
        if (num_values == 1) state_.insert(std::make_pair(name, GlState::GlStateVariable(static_cast<bool>(v[0]))));
    }
    ...
    GlCapabilities::GlCapabilities() {
        ...
        initGlParameter<int>(GL_MAX_VIEWPORT_DIMS, 2);
