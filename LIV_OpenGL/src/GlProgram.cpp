...
namespace LIV_OpenGL{
    ...
    void GlProgram::attach(const LIV_OpenGL::GlShader &shader) {
        if(shaders_.find(shader.type_) != shaders_.end())
            shaders_.erase(shader.type_);
        shaders_.emplace(shader.type_, shader);
    }

    void GlProgram::attach(const LIV_OpenGL::GlTransformFeedback& feedback) {
        feedbacks_.push_back(feedback);
    }

    void GlProgram::link() {
        if(shaders_.find(SHADER_TYPE::VERTEX_SHADER) == shaders_.end() ||
           shaders_.find(SHADER_TYPE::FRAGMENT_SHADER) == shaders_.end())
        ...
        // produce warnning for unmatched inputs or outputs.
        std::vector<SHADER_TYPE> ordering{SHADER_TYPE::VERTEX_SHADER, SHADER_TYPE::GEOMETRY_SHADER, SHADER_TYPE::FRAGMENT_SHADER};
        ...
        for(uint32_t i = 0; i < ordering.size(); ++i){
            ...
            if(shader.inAttribs_.size() == 0) continue;
            ...
            for(auto attrib : shader.inAttribs_){
                ...
                for(int32_t j = i - 1; j >= 0; --j){
                    ...
                    for(auto prevAttrib : prevShader.outAttribs_)
                        // 这里和之前有点不一样，感觉作者写错了
                        if(prevAttrib.type == attrib.type && prevAttrib.name == attrib.name) unmatched = false;
                        if(!unmatched) break;
                }
                if(unmatched){
