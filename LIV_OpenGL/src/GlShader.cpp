...
namespace LIV_OpenGL{
    GlShader::GlShader(const LIV_OpenGL::SHADER_TYPE &type, const std::string &source, bool useCache) {
    ...
    SHADER_TYPE GlShader::type() const{
        return type_;
    }

    GlShader GlShader::fromeFile(const LIV_OpenGL::SHADER_TYPE &type, const std::string &filename) {
    ...
    GlShader GlShader::fromeCache(const LIV_OpenGL::SHADER_TYPE &type, const std::string &filename) {
    ...
    std::string GlShader::readSource(const std::string &file_name) {
        std::ifstream in_file(file_name);
        if(!in_file.is_open()) throw GlShaderError("Unable to open shader file: " + file_name);
    ...
    std::string GlShader::preProcess(const std::string &source) {
        ...
        while(!in.eof()){
            ...
            if(tokens.size() == 2 && tokens[0] == "#include"){
                if(tokens.size() != 2)
                    throw GlShaderError("Filename of include file missing in line " + std::to_string(line_no));
        ...
        if(out.str().size() == 0)
            std::cerr << "Warning empty shader source" << std::endl;
