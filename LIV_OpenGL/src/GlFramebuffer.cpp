...
namespace LIV_OpenGL{
    ...
    GlFramebuffer::GlFramebuffer(uint32_t width, uint32_t height, LIV_OpenGL::FRAMEBUFFER_TARGET target):
    ...
    GlFramebuffer::~GlFramebuffer() {
        if(boundFramebuffer_ == id_){
            release();
            assert((boundFramebuffer_ != id_) && "Framebuffer object bound.");
        }
        attachments_.clear();
    }
    ...
    void GlFramebuffer::attach(FRAMEBUFFER_ATTACHMENT target, GlTexture& texture){
        ...
        attachments_[target] = texture.ptr_;
        valid_ = (glCheckFramebufferStatus(target_) == GL_FRAMEBUFFER_COMPLETE);
        releaseTransparently(old_buffer);
    }

    void GlFramebuffer::attach(FRAMEBUFFER_ATTACHMENT target, GlTextureRectangle& texture){
        if(texture.width() < width_){
            std::stringstream error;
            error << "Texture's width should be at least " << width_;
            throw GlFramebufferError(error.str());
    ...
    void GlFramebuffer::attach(LIV_OpenGL::FRAMEBUFFER_ATTACHMENT target, GlRenderbuffer &buffer) {
        ...
        attachments_[target] = buffer.ptr_;
        valid_ = (glCheckFramebufferStatus(target_) == GL_FRAMEBUFFER_COMPLETE);
